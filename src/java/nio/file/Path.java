...
// 文件系统中的路径，文件系统都是Tree或者层级结构来组织文件的，任何一个节点可以是一个目录或者一个文件，在NIO2中称为Path，这和原来的File有很多相似之处，只是Path具有更多的表述语义
public interface Path extends Comparable<Path>, Iterable<Path>, Watchable {
    ...
    /**
     * 比如“/data/logs/./web.log”、“/data/logs/../db”这种包含“冗余”路径的，有时候我们需要转换为正常路径语句，则使用“Path.normalize()”方法，无比的清爽清洁。
    ...
    // 获取相对路径，“/data”与“/data/logs/p1”的相对路径为“logs/p1”，反之为“../../”
    ...
    // 如果路径为相对路径，则转换为绝对路径，对于JAVA程序而言，起始路径为classpath
    Path toAbsolutePath();
    ...
    // 比较重要的方法，不过它会对文件是否存在、访问权限进行检测，需要捕捉异常
    // 1、首先检测文件是否存在、是否有权限；
    // 2、如果path为相对路径，则将会转换为绝对路径
    // 3、如果是符号连接文件（软连接），则获取其实际target路径
    // 4、如果路径中包含“冗余”，则移除，例如“/data/logs/../web.log”
    ...
    /**
     * 如果文件需要被【外部资源】访问（resource），你可以通过Path.toUri()来转换，path对应的文件或者目录可以不存在，此方法不会check异常
     * Path path = Paths.get("/data/logs/web.log");
