...
class WinNTFileSystem extends FileSystem {
    ...
    // 斜杠符号
    private final char slash;       // 【标准】路径内部的分隔符：Windows系统上是'\'，类Unix系统上是'/'
    // 与slash相反的斜杠符号
    private final char altSlash;    // 【其他可用】的路径内部的分隔符：Windows系统上是'/'，类Unix系统上是'\'
    // 分号
    private final char semicolon;   // 【路径之间】的分隔符：Windows系统上是';'，类Unix系统上是':'
    // ?
     ...
     * 3.忽略末尾的路径分隔符
     */
    // 路径的标准化，例如D://////\\\\\\/testFile\\\\\\\\\///////\\wwww.txt，去除不必要的分隔符或者把错误的分隔符进行替换等按照一定的规则
    // 整理出来一个合理的路径，这就是标准化
    @Override
     ...
     */
    // 要求传递进去的两个字符串都是normal的，虽然说两个都是normal的，但是两个拼接起来的情况需要处理，
    // 比如子路径以slash开头，丢弃它的头部，所以子路径从第二个位置开始
    // 用于合并路径
    @Override
    ...
    // 先根据驱动盘获取对应的驱动盘索引
    // 然后尝试从缓存中读取，如果读取到直接返回
    // 如果获取不到，使用本地方法获取，然后再缓存起来
