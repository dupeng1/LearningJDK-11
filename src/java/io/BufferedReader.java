 ...
 * 带有内部缓存区的字符输入流
 *
 * 读取数据时，会先从【包装的输入流】中读取数据，然后暂存在【内部缓冲区】中，
 * 最后【对外开放缓冲区】，避免了【频繁】读取输入流造成的低效问题。
 */
public class BufferedReader extends Reader {
     ...
     */
    // 返回从字符输入流中【读取的一个char】，返回-1表示读取失败
     ...
     */
    // 从输入源【读取len个字符】存入cbuf的off处，返回实际读到的字符数
     ...
     */
    // 从输入源读取len个字符存入cbuf的off处，返回实际读到的字符数，
    // 如果内部缓冲区没有可读字符则读取输入流到内部缓冲区，从内部缓冲区读取字符到数组
