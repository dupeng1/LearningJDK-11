 ...
 */
/*
 * 1、统一资源标识符URI，包括URL（统一资源定位符）与URN（统一资源名），实际上使用的URI大多是URL，但大多数规范和标准都是用URI定义的
 * 2、统一资源标识符是采用一种特定语法标识一个资源的字符串，所标识的资源可能是服务器上的一个文件，也可能是一个邮件地址、
 * 新闻消息、图书、人名、Internet主机等内容
 * 3、只标识资源
 * 4、URI类完全有关于资源的标识和URI的解析，它没有提供方法来获取URI标识资源
 * 5、相比URL类，URI类与相关的规范更一致
 * 6、URI对象可以表示相对URI，URL类在存储URI之前会将其绝对化
 *
...
public final class URI implements Comparable<URI>, Serializable {
     ...
     */
    // 通过模式和模式特定部分创建URI。如果模式为空，则创建一个相对URI。模式特定部分取决于URI模式的语法
     ...
     */
    // 返回构造的URI对象
    // 如果这个URI证明是不正确的，会抛出IllegalArgumentException异常，这是一个运行时异常，不需要显示声明或捕获这个异常
     ...
     */
    // 规范化：返回当前URI的规范化形式(包括对路径内分隔符的转换，以及对"."和".."的解析)
     ...
     */
    // 相对化：如果当前URI中包含了otherUri，则返回一个相对URI，
    // 通过该相对URI，可以从当前URI访问到otherUri，否则，直接返回otherUri
    // 调用uRI包含参数URI的一部分
    public URI relativize(URI otherUri) {
        return relativize(this, otherUri);
    }
     ...
     * @throws IllegalArgumentException If the given string violates RFC&nbsp;2396
     */
    // 绝对化：基于当前URI解析otherUri；
    // 如果otherUri是相对URI，则返回"当前URI+otherUri"，
    // 如果otherUri是绝对URI，原样返回
    // 如果调用URI本身包含绝对URI
    // 如果调用URI本身不包含绝对URI，尽可能解析URI，返回新的相对URI
    public URI resolve(String otherUri) {
        return resolve(URI.create(otherUri));
    }
    ...
    // 如果otherUri是相对URI，则返回"当前URI+otherUri"，
    // 如果otherUri是绝对URI，原样返回
    public URI resolve(URI otherUri) {
        return resolve(this, otherUri);
    }
     ...
     */
    // 判断当前URI是否不透明(包含路径)
    // 透明，层次URI，返回false，层次URI的所有部分都有相应的获取方法
    // 不透明，不分层，返回true，只能得到模式、模式特定部分、片段
     ...
     */
    // 强制解析登录信息
    // 不总是在开始就检测授权机构部分中的语法错误，不做这个检测的直接后果是，一般无法返回授权机构的各个部分：端口、主机、用户信息
    // 在这种情况下，强制重新解析授权机构，原来的URI没有改变，但返回的URI对应用户信息、主机和端口有单独的授权机构部分
    public URI parseServerAuthority() throws URISyntaxException {
        // 存在authority部分，但是之前没有解析出host，则需要进一步解析
        if((authority != null) && (host == null)) {
            new Parser(toString()).parse(true);
        }
        
        /*
         * We could be clever and cache the error message and index from the exception thrown during the original parse,
         * but that would require either more fields or a more-obscure representation.
         */
        return this;
    }
    ...
    public int compareTo(URI that) {
        ...
        if((this.host != null) && (that.host != null)) {
            ...
            // If one or both authorities are registry-based then we simply compare them in the usual, case-sensitive way.
            // If one is registry-based and one is server-based then the strings are guaranteed to be unequal,
            // hence the comparison will never return zero and the compareTo and equals methods will remain consistent.
    ...
    private static URI resolve(URI base, URI child) {
        ...
        if((child.scheme == null) && (child.authority == null) && child.path.isEmpty() && (child.fragment != null) && (child.query == null)) {
    ...
    /**
     * If both URIs are hierarchical, their scheme and authority components are identical,
     * and the base path is a prefix of the child's path, then return a relative URI that,
     * when resolved against the base, yields the child;
    ...
    private static void maybeAddLeadingDot(char[] path, int[] segs) {
        ...
        if(p >= path.length || path[p] == '\0') {
            return; // No colon in first segment, so no "." needed
        }
    ...
    // We here define a set of mask pairs to aid in enforcing these restrictions.
    // Each mask pair consists of two longs, a low mask and a high mask.
    // Taken together they represent a 128-bit mask, where bit i is set if the character with value i is permitted.
    //
    // This approach is more efficient than sequentially searching arrays of permitted characters.
    // It could be made still more efficient by precompiling the mask information
    // so that a character's presence in a given mask could be determined by a single table lookup.
    ...
    // Compute a low-order mask for the characters between first and last, inclusive
    ...
    // Compute a high-order mask for the characters between first and last, inclusive
    ...
    /**
     * The zero'th bit is used to indicate that escape pairs and non-US-ASCII characters are allowed;
    ...
    // Quote any characters in s that are not permitted by the given mask pair
    ...
    // Encodes all characters >= \u0080 into escaped, normalized UTF-8 octets, assuming that s is otherwise legal
    ...
    // This method was introduced as a generalization of URI.decode method to provide a fix for JDK-8037396
    ...
    private class Parser {
        ...
        void parse(boolean rsa) throws URISyntaxException {
            ...
            // 在input之中查找首个":"的位置，如果查找中遇到了"/?#"，则返回-1
            int p = scan(0, n, "/?#", ":");
            
            // 且':'在input[p]处
            /*
             * 如果遇到":"之前，先遇到了"/?#"，说明没有scheme；
             * 或者，既没遇到":"，也没找到"/?#"；
             * 此时，
             */
            if((p<0) || !at(p, n, ':')) {
        ...
        // 判断c是否在input[start]处，前提是start<end
        private boolean at(int start, int end, char c) {
            return (start<end) && (input.charAt(start) == c);
        }
        
        /** Tells whether start + s.length() < end and, if so, whether the chars at the start position match s exactly */
        // 判断s是否在input[start]处
        ...
        /**
         * Scan a specific char: If the char at the given start position is equal to c,
         * return the index of the next char; otherwise, return the start position.
         */
        // 如果c在input[start]处，则返回start+1，否则返回start
        private int scan(int start, int end, char c) {
            // 判断c是否在input[start]处
            boolean at = at(start, end, c);
            if(at) {
                return start + 1;
            }
         ...
         * Stop at the first char in the err string (in which case -1 is returned),
         * or the first char in the stop string (in which case the index of the preceding char is returned),
         * or the end of the input string (in which case the length of the input string is returned).
         * May return the start position if nothing matches.
         */
        // 在input的[start, end)之间查找首个stop的索引，如果查找中遇到了err，则返回-1
        ...
        /**
         * Scan forward from the given start position.
         * Stop at the first char in the stop string
         * (in which case the index of the preceding char is returned),
         * or the end of the input string (in which case the length of the input string is returned).
         * May return the start position if nothing matches.
         */
        // 返回stop中的字符在input的[start, end)范围内首次出现的索引
        private int scan(int start, int end, String stop) {
            int p = start;
            
            while(p<end) {
                if(stop.indexOf(input.charAt(p)) >= 0) {
                    break;
                }
                p++;
            }
        ...
        /**
         * Scan a potential escape sequence, starting at the given position, with the given first char (i.e., charAt(start) == c).
         * This method assumes that if escapes are allowed then visible non-US-ASCII chars are also allowed.
         */
        // 扫描转义符。如果存在转义符，返回转义符之后的字符的索引
        private int scanEscape(int start, int n, char first) throws URISyntaxException {
            // 尝试匹配uri中的转义符，如%2C
            if(first == '%') {
                // Process escape pair
                if((start + 3<=n) && match(input.charAt(start + 1), L_HEX, H_HEX) && match(input.charAt(start + 2), L_HEX, H_HEX)) {
                    return start + 3;
                }
                
                fail("Malformed escape pair", start);
                
            } else if(first>128 // 非ASCII字符
                && !Character.isSpaceChar(first)        // 如果first不是Unicode空格
                && !Character.isISOControl(first)) {    // 如果first不是ISO控制字符
         ...
         * We allow an empty authority component as long as it's followed by
         * a non-empty path, query component, or fragment component.
         * This is so that URIs such as "file:///foo/bar" will parse.
         * This seems to be the intent of RFC2396, though the grammar does not permit it.
         * If the authority is empty then the userInfo, host, and port components are undefined.
         *
         * DEVIATION from RFC2396:
         * We allow empty relative paths.
         * This seems to be the intent of RFC2396, but the grammar does not permit it.
         * The primary consequence of this deviation is that "#f" parses as a relative URI with an empty path.
         */
        private int parseHierarchical(int start, int n) throws URISyntaxException {
            ...
            if(at(p, n, '/') && at(p + 1, n, '/')) {
                ...
                // 在input[p, n)范围内查找"/?#"中的字符，返回首次相遇的索引
                int q = scan(p, n, "/?#");
                
                // 如果找到了"/?#"，且p到q之间有数据，说明可以解析authority了
                if(q>p) {
                    // 解析authority
         ...
         *
         * Ambiguity:
         * An authority that is a registry name rather than a server might have a prefix that parses as a server.
         * We use the fact that the authority component is always followed by '/'
         * or the end of the input string to resolve this:
         * If the complete authority did not parse as a server then we try to parse it as a registry name.
         *
         * authority = [userinfo@]host[:port]
         */
        // 解析authority
        private int parseAuthority(int start, int n) throws URISyntaxException {
            ...
            // 在input[start, n)范围内查找"]"中的字符，返回首次相遇的索引
            int i = scan(start, n, "]");
            int j;
            
            // 处理IP6地址，形如：[2000::1:2345:6789:abcd]，完整形式是2000:0000:0000:0000:0001:2345:6789:abcd
            if(i>start) {
                j = scan(start, n, L_SERVER_PERCENT, H_SERVER_PERCENT);
                ...
                j = scan(start, n, L_SERVER, H_SERVER);
                serverChars = (j == n);
            }
            
            j = scan(start, n, L_REG_NAME, H_REG_NAME);
            regChars = (j == n);
            ...
            if(serverChars) {
                /*
                 * Might be (probably is) a server-based authority, so attempt to parse it as such.
                 * If the attempt fails, try to treat it as a registry-based authority.
                 */
                try {
                    ...
                    if(requireServerAuthority) {
                        // If we're insisting upon a server-based authority, then just re-throw the exception
                        throw x;
                    } else {
                        // Save the exception in case it doesn't parse as a registry either
                        ex = x;
                        q = start;
                    }
                }
        ...
        private int parseServer(int start, int n) throws URISyntaxException {
            ...
            // 在[p, n)之间查找首个"@"的索引，如果查找中遇到了"/?#"，则返回-1
            q = scan(p, n, "/?#", "@"); // userinfo
        ...
        // Take an IPv4 address: Throw an exception if the given interval contains anything except an IPv4 address
        ...
        // Attempt to parse an IPv4 address, returning -1 on failure but allowing the given interval to contain [:<characters>] after the IPv4 address.
        private int parseIPv4Address(int start, int n) {
            ...
            if(p>start && p<n) {
                // IPv4 address is followed by something - check that it's a ":" as this is the only valid character to follow an address.
                if(input.charAt(p) != ':') {
                    p = -1;
                }
            }
