 ...
 */
/*
 * 1、统一资源定位符URL，属于URI的一种分类
 * 2、既能标识资源，又能获取资源
 * 3、像绝对化和编码等操作，在选择时首选URI类
 * 4、如果需要把URL存储在一个散列表或其他数据结构中，应当首选URI，因为它的equals方法不会阻塞
 * 5、URL类应当用于从服务器下载
 *
...
public final class URL implements Serializable {
    ...
    private transient String userInfo;  // 用户信息，授权机构一部分
    // The host name to connect to
    private String host;                // 主机地址，授权机构一部分
    // The protocol port to connect to
    private int port = -1;              // 端口号，授权机构一部分
     ...
     */
    // 返回用户信息(userinfo)
    // 有些URL包括用户名，有时甚至会又口令信息。这些信息位于模式之后，而且在主机之前
     ...
     */
    // 返回端口(port)
    // 如果URL中没有指定端口，返回-1，表示这个URL没有显示指定端口，将使用该协议的默认端口
    public int getPort() {
        return port;
    }
     ...
     */
    // 返回文件名
    // 主机后的第一个斜线（/）一直到开始片段标识符的#号之前的字符，被认为是文件部分
     ...
     */
    // 打开URL资源连接
    // 除了访问原始的文档外，还可以访问这个协议指定的元数据，如果模式是HTTP或HTTPS，允许访问HTTP首部以及原始HTML
     ...
     */
    // 返回指向URL资源的【输入流】，可以从中读取数据
    // 连接到URL所引用的资源，在客户端和服务器之间完成必要的握手，返回一个InputStream，可以由此读取数据
    // 从这个InputStream获得的数据是URL引用的原始内容，不包括任何HTTP首部或者协议有关的任何内容
     ...
     */
    // 返回【目标资源的内容】，返回的形式取决于资源的类型(不一定总是输入流)
    // 向URL请求内容
    // 如果URL指示某种文本（ASCII或HTML），返回的对象通常是某种InputStream
    // 如果URL指示一个图像（GIF或JPEG），返回一个Java.awt.ImageProducer
    // 很难预测将获得哪种对象，用instanceof操作符很容易检查，对于读取文件或显示一个图像应该足够了
    ...
    // 与equals基本相同，这里也包含DNS查询，但是该方法补考虑片段标识符
     ...
     * @see java.net.URLStreamHandler#toExternalForm(java.net.URL)
     */
    // 绝对URL，显示调用该方法并不常见，显示（打印）语句会隐式调用该方法
     ...
     * {@code false} otherwise.
     */
    // 两个URL都指向相同主机、端口和路径上的相同资源，而且有相同的片段标识和查询字符串
    // 会尝试用DNS解析主机，来判断两个主机是否相同
