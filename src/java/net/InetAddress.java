 ...
 */
/*
 * InetAddress代表一个【IP地址】，包括IP4和IP6。
 *
 * IP地址由一串无符号字节组成，IP4是用点号分割的4个无符号字节（共计32位），IP6是用冒号分割的8个区块，每个区块是4个十六进制数字（共计128位）。
 *
 * java对IP地址的高层次表示，包括一个【主机名】和一个【IP地址】，它提供了获取和操作IP地址的方法，可用于网络编程、服务器配置、主机管理等场景
 * 有两个子类Inet4Address、Inet6Address，分别用于封装4个字节的ip地址和16个字节的ip地址，InetAddress内部对地址数字进行隐藏
 */
public class InetAddress implements Serializable {
     ...
     * @since 1.4
     */
    // 用给定【主机名称】与【主机地址】创建一个InetAddress实例
     ...
     */
    // 返回【本地主机】地址
    // 获取本机对应的InetAddress对象，尝试连接DNS来得到一个真正的【主机名和IP地址】
    // 如果没有连接到Internet，系统也没有固定IP地址或域名，可能会看到域名为localhost，IP地址为127.0.0.1
     ...
     */
    // ▶ 1-1-1-1 根据【主机名称或主机IP】，创建/查找其对应的InetAddress实例（有多个实例时只选择第一个）
    // 1、host是主机名称，会建立与本地DNS服务器的一个连接，来查找名字和数字地址
    // 2、host是ip地址，不检查DNS。对象的主机名初始设置为host，只有当请求主机名时，才会真正完成主机名的DNS查找
    // 如果DNS无法找到，那么主机名会保持为最初的host，不会抛出异常
     ...
     * @see SecurityManager#checkConnect
     */
    // ▶ 1-1-1 根据【主机名称或主机IP】，创建/查找其对应的InetAddress实例，得到一个主机的所有地址
     ...
     */
    // 获取当前网络地址的【主机名】
    // 与getHostName类似，在与DNS联系方面更积极一些，getHostName在不知道主机名才会联系DNS，getCanonicalHostName知道
    // 主机名也会联系DNS，可能会替换原来缓存的主机名
     ...
     */
    // 返回【IP地址】的【字节形式】
    // 之所以要查看IP地址的原始字节，一个原因是想要确定地址类型，根据返回数组的字节数可以确定处理的是IPv4还是IPv6地址
     ...
     */
    // 判断当前地址是否为【本地环回地址】
    // 环回地址直接在IP层连接同一台计算机，而不使用任何物理硬件，在IPv4中统配地址是127.0.0.1
     ...
     */
    // 判断当前地址是否为【通配地址】
    // 通配地址可以匹配本地系统中的任何地址，在IPv4中统配地址是0.0.0.0
     ...
     */
    // 判断当前地址是否为【站点本地地址】
    // Ipv6本地网站地址，与本地连接地址相似，可以由路由器在网站或校园内转发，但不应该转发到网站以外
     ...
     */
    // 判断当前地址是否为【组播地址】
    // 组播地址会将内容广播给所有预定的计算机，在IPv4中，组播地址都在224.0.0.0到239.255.255.255范围内
     ...
     * @since 1.4
     */
    // 判断当前地址是否为【节点（或接口）本地范围的组播地址】，本地接口组播地址
     ...
     * @since 1.4
     */
    // 判断当前地址是否为【链路本地范围的组播地址】，子网范围组播地址
     ...
     * @since 1.4
     */
    // 判断当前地址是否为【站点本地范围的组播地址】，网站范围组播地址
     ...
     * @since 1.4
     */
    // 判断当前地址是否为【机构本地范围的组播地址】，组织范围组播地址
     ...
     */
    // 只根据IP地址来计算，不考虑主机名，如果两个对象有相同的地址（IP），就会有相同的散列码，即使他们的主机名有所不同
    @Override
    public int hashCode() {
        return -1;
    }
     ...
     */
    // 返回对象的简单文本表示，字符串有如下形式：主机名/点分四段地址
    @Override
